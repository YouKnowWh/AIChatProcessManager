...
const workspace = path.resolve("outputs/manual-aichat-complete-ppt/presentations/aichat-process-manager");
...
const slidesSeed = [
  {
    ...
    stats: [["13", "核心数据表"], ["10+", "API 模块"], ["2", "用户角色"], ["5", "过程维度"]],
  },
  {
    no: 2,
    kind: "thesis",
  ...
  },
  {
    ...
    items: ["背景与意义", "技术框架", "用户需求与功能设计", "数据库设计", "系统实现与运行验证", "总结与反思"],
  },
  {
    no: 4,
    kind: "contrast",
    ...
    leftTitle: "普通聊天记录",
    left: ["用户问题", "AI 最终回复", "少量时间戳"],
    rightTitle: "过程管理平台",
    right: ["消息正文与多内容块", "reasoning 可见性控制", "工具调用参数与状态", "工具结果与错误信息", "模型 token / 时延 / finish reason"],
  ...
  },
  {
    ...
    roles: [
      ["普通用户", ["登录注册", "创建 AI 角色", "发起会话", "收藏消息", "提交反馈", "查看个人统计"]],
      ["管理员", ["用户状态管理", "角色状态治理", "消息治理", "反馈处理", "系统日志", "平台统计"]],
    ],
  ...
  },
  {
    ...
    steps: ["JWT 登录", "选择角色", "创建会话", "发送消息", "AI 回复", "过程入库", "收藏/反馈", "后台治理"],
  ...
  },
  {
    ...
    groups: [
      ["身份与角色", ["users", "ai_characters"]],
      ["会话与消息", ["conversations", "messages", "message_contents"]],
      ["AI 过程链", ["message_reasoning", "tool_calls", "tool_results", "message_metadata"]],
      ["交互与治理", ["message_favorites", "feedbacks", "system_logs", "audit_records"]],
    ],
  },
  {
    no: 10,
  ...
  },
  {
    ...
    rows: [
      ["messages", "conversation_id, parent_message_id, sender_type, role, sequence_number", "承载会话顺序与父子关系，并作为其他过程表的主关联中心"],
      ["message_contents", "content_type, content, sort_order", "支持多段正文排序展示，便于后续扩展富文本、多模态或附件内容"],
      ["message_reasoning", "reasoning_content, visibility", "控制过程信息可见范围，兼顾展示需要与敏感信息隔离"],
      ["tool_calls", "tool_name, arguments, status, called_at", "记录工具调用参数、调用时点与执行状态，支撑过程复盘"],
      ["message_metadata", "model_name, tokens, duration_ms, finish_reason", "支撑成本统计、性能分析、模型对比与质量评估"],
    ],
  },
  {
    no: 12,
    ...
    lanes: [
      ["用户侧", "只能访问自己的会话、消息、收藏和反馈"],
      ["管理员侧", "可进入后台治理用户、角色、消息、反馈、日志和统计"],
      ["数据侧", "角色删除改为禁用，历史会话仍保持引用完整"],
    ],
  },
  {
    no: 13,
    ...
    modules: ["登录注册", "角色列表", "会话列表", "聊天详情", "过程详情弹窗", "收藏中心", "个人资料"],
  ...
  },
  {
    ...
    modules: ["用户管理", "角色管理", "消息管理", "反馈管理", "系统日志", "统计看板"],
  ...
  },
  {
    ...
    steps: ["收藏消息", "提交反馈", "关联 message_id", "关联 character_id", "后台处理", "统计复盘"],
  ...
  },
  {
    ...
    points: [
      ["system_logs", "记录 action、target_type、target_id、detail、ip_address"],
      ["audit_records", "记录 admin_id、target、action 与 comment"],
      ["status fields", "users / characters / conversations / messages 均具备状态位"],
    ],
  },
  {
    no: 17,
    ...
    stages: ["请求上下文", "角色提示词", "模型调用", "失败回退", "过程映射", "消息入库"],
  ...
  },
  {
    ...
    checks: [
      ["后端导入", "路由注册与 ORM 模型可加载"],
      ["前端构建", "Vue 3 + TypeScript + Vite production build"],
      ["API 冒烟", "健康检查、鉴权、消息、收藏、反馈路径"],
      ["数据核验", "中文 seed 数据与 utf8mb4 编码"],
    ],
  },
  {
    no: 20,
    ...
    fixes: [
      ["路由前缀", "messages / users / favorites / feedbacks 统一为 /api"],
      ["中文编码", "MySQL、SQLAlchemy、seed 统一 utf8mb4"],
      ["收藏状态", "消息列表返回 is_favorited，切换页面后保持状态"],
      ["权限隔离", "用户不能读取、收藏或反馈别人的消息"],
      ["数据保留", "角色删除改为禁用，保护历史会话引用"],
      ["配置健壮性", "DEBUG=release 可解析，模型失败可回退"],
      ["索引与约束", "主外键关联、状态字段与排序字段共同保证查询效率与结构稳定性"],
    ],
  },
  {
    no: 21,
    ...
    stats: [["13", "ORM / DDL 核心表"], ["10+", "后端路由模块"], ["18+", "前端视图"], ["6", "后台治理页"], ["5", "AI 过程数据维度"]],
  },
  {
    no: 22,
    kind: "risk",
    ...
    risks: [
      ["推理敏感性", "visibility 字段控制 reasoning 暴露范围"],
      ["工具结果可信度", "tool_results 保留 is_error 与原始 JSON"],
      ["历史数据破坏", "角色禁用替代物理删除"],
      ["模型接入不稳定", "DeepSeek 调用失败回退模拟 AI"],
      ["中文数据质量", "数据库连接和 seed 均使用 utf8mb4"],
    ],
  },
  {
    no: 23,
    ...
    steps: ["登录 user1 / admin", "查看角色与会话", "进入聊天详情", "展开过程数据", "收藏并返回验证", "提交反馈", "进入管理后台", "展示日志与统计"],
  },
  {
    no: 24,
    kind: "roadmap",
  ...
  },
  {
    ...
    bullets: ["完整链路：问题、回复、推理、工具、结果、元数据都可保存。", "工程闭环：前端、后端、数据库、Docker 与文档形成可运行系统。", "治理闭环：用户行为、反馈、日志、审核与统计都进入后台。", "扩展闭环：真实模型、流式响应、质量分析可沿当前模型继续实现。"],
  },
  {
    no: 26,
    kind: "er",
  ...
  },
  {
    ...
    body: "感谢观看",
    stats: [["Vue 3", "Frontend"], ["FastAPI", "Backend"], ["MySQL", "Database"], ["Docker", "Runtime"]],
  },
];

const reorderedNos = [
  1, 3, 4, 2, 7, 8, 5, 6, 12, 13, 15, 9, 26, 10, 11, 14, 16, 17, 18, 19, 20, 23, 21, 22, 24, 25, 27,
];

const slideMap = new Map(slidesSeed.map((slide) => [slide.no, slide]));
const slides = reorderedNos.map((oldNo, index) => ({
  ...slideMap.get(oldNo),
  sourceNo: oldNo,
  no: index + 1,
}));
...
export const assets = {
  processChain: "${path.join(workspace, "assets/10-process-chain.png")}",
  adminConsole: "${path.join(workspace, "assets/14-admin-console.png")}",
  verificationBoard: "${path.join(workspace, "assets/19-verification-board.png")}",
  erDiagram: "${path.join(workspace, "assets/ig_02afd613b312915a016a27d0ca305c8191a3290a7590be1f0d.png")}",
};
...
for (const spec of slides) {
  const n = String(spec.no).padStart(2, "0");
  const module = `import { renderSlide } from "./common.mjs";\n\nexport async function slide${n}(presentation, ctx) {\n  return renderSlide(presentation, ctx, ${spec.no});\n}\n`;
  await fs.writeFile(path.join(slidesDir, `slide-${n}.mjs`), module, "utf8");
}

const claimSpine = [
  "thesis: AIChatProcessManager is an AI conversation governance platform centered on process data, not a generic chat UI.",
  "audience: course defense reviewers who need to see problem framing, system design, implementation completeness, and demo path.",
  "arc: missing process evidence -> structured data chain -> full-stack implementation -> governance and verification -> demo and roadmap.",
  "",
  ...slides.map((s) => `${String(s.no).padStart(2, "0")}. ${s.kicker}: ${s.title}`),
].join("\n");

const designSystem = [
  "slide size: 1280x720.",
  "background: alternating deep ink technical pages and warm paper evidence pages.",
  "type: PingFang SC for Chinese readability; bold title hierarchy, compact labels.",
  "palette: deep ink, warm paper, cyan primary, mint success, amber proof, coral risk, violet admin.",
  "layout rhythm: cover, thesis, agenda, contrast, lanes, process chain, architecture map, table, module grids, roadmap.",
  "animation plan: post-export PowerPoint transitions added as smooth fade/push/wipe variants when compatible.",
].join("\n");

const contactPlan = slides.map((s) => `${String(s.no).padStart(2, "0")} ${s.kind}: ${s.title}`).join("\n");

await fs.writeFile(path.join(workspace, "claim-spine.txt"), claimSpine, "utf8");
await fs.writeFile(path.join(workspace, "design-system.txt"), designSystem, "utf8");
await fs.writeFile(path.join(workspace, "contact-sheet-plan.txt"), contactPlan, "utf8");
await fs.writeFile(path.join(workspace, "profile-plan.txt"), "task mode: create\nprimary deck-profile: engineering-platform\nproof objects: architecture map, data model, process chain, API surface, permission lanes, verification and roadmap\n", "utf8");
await fs.writeFile(path.join(workspace, "source-notes.txt"), "Sources: AIChatProcessManager.pptx, README.md, docs/database_design.md, repository file inventory. No external identity assets used.\n", "utf8");
